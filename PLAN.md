...
1. **查询优化器（含意图理解一体化）**：把 chat 意图路由升级为单次 LLM 调用完成 意图判定 + 结构化参数抽取（年份/数量/来源，替代现在丢弃参数的浅层路由）+ 翻译（中→英学术表述）/ 展开（缩写→全称、口语→术语）/ 分解（复合问题拆 1~3 个子查询，复用现有去重合并）；**失败兜底：优化失败原样直搜、路由失败退回关键词启发式，绝不阻塞**；分不清意图时向用户澄清一句而非瞎猜；
2. **会话域守卫与探索引导**：意图枚举扩展 `explore`（用户不知道看什么时，基于库内论文主题分布给 2~3 个可探索方向供选择）与 `off_topic`（与科研/论文完全无关的请求——闲聊、写周报、通用编程等——礼貌拒绝并把话题引回论文场景）。判定标准写入分类器 prompt：科研/文献相关一律接待（库外问题由 qa 链的「库里没有相关内容」诚实兜底），仅纯无关才拒绝；启发式兜底路径保守接待、不误拒；
...
6. **能力引导**：`pa chat` 欢迎语列出自有能力与示例话术，降低发现成本。
- **不做**（放二期）：Agent 主动性（主动追问/主动建议/跨功能串联）——依赖会话记忆（SqliteSaver）与工具契约，现在硬做成关键词规则是伪智能；联网的论文推荐与「找相关论文」——归二期引用图谱。
- **验收**：中文主题词能搜到相关英文论文；「找 2023 年后的 XX 综述」这类带条件的自然语言参数被正确抽取；不相关候选被预筛剔除；任一源故障不影响其它源出结果（用新契约重构验证）；优化器失败时搜索照常工作；域外请求（如「帮我写周报」）被拒绝并引导回论文场景，「不知道看什么」能得到基于库内的方向引导。
